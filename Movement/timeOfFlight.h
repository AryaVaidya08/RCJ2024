...
uint32_t _tofRawValue(int select) {
  tcaselect(select);
  return tof.readRangeSingleMillimeters();
}

uint32_t _tofCalibrated(int select) {
  uint32_t dist = 0;
  uint32_t cal = 0;
  const uint32_t max_dist = 250;
  switch (select) {
    case 0:
      {
        tcaselect(0);
...
uint32_t tofCalibrated(int select, int samples = 1, int* invalid_count = nullptr) {
  uint64_t dist = 0;
  uint32_t val = 0;
  if(invalid_count)
    invalid_count = 0;
